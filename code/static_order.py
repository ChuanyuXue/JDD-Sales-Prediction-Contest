...
for d in date_list:
    history = order[order['ord_dt'] <= d]

    #sale_amt
    sale_amt = history.groupby('shop_id', as_index=False).agg({'sale_amt':['sum','mean','median','max','min', 'std']})
    sale_amt.columns = ['shop_id','sale_amt_sum','sale_amt_mean','sale_amt_median','sale_amt_max', 'sale_amt_min','sale_amt_std']
    #offer_amt
    offer_amt = history.groupby('shop_id', as_index=False).agg({'offer_amt':['sum', 'mean', 'median', 'max', 'min', 'std']})
    ...
    #offer_cnt
    offer_cnt = history.groupby('shop_id', as_index=False).agg({'offer_cnt':['sum', 'mean', 'median', 'max', 'min', 'std']})
    ...
    #rtn_cnt
    rtn_cnt = history.groupby('shop_id', as_index=False).agg({'rtn_cnt':['sum', 'mean', 'median', 'max', 'min', 'std']})
    ...
    #ord_cnt
    ord_cnt = history.groupby('shop_id', as_index=False).agg({'ord_cnt':['sum', 'mean', 'median', 'max', 'min', 'std']})
    ...
    #usr_cnt
    user_cnt = history.groupby('shop_id', as_index=False).agg({'user_cnt':['sum', 'mean', 'median', 'max', 'min', 'std']})
...
#输出
feature.to_csv('static_order.csv', index=False)